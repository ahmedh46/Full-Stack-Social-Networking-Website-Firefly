...
class Message {
    ...
    public function getLatestMessage($userLoggedIn, $user2) {
        ...
        $query_to_show_name = mysqli_query($this->con, "SELECT first_name FROM users WHERE user_name = '$user2'");
        $row2 = mysqli_fetch_array($query_to_show_name);
        $row2_result = $row2['first_name'];
        $row = mysqli_fetch_array($query);
        $sent_by = ($row['user_to'] == $userLoggedIn) ? "$row2_result said: " : "You said: ";
    ...
    public function getConvos() {
        ...
        foreach ($convos as $username) {
            ...
            $return_string .= "<a href='messages.php?u=$username'> <div class='user_found_messages'>
								...
								<p id='grey' style='margin-bottom: 0;'>" . $latest_message_details[0] . $split . " </p>
								</div>
								</a>";
        }
    ...
    public function getConvosDropdown($data, $limit) {
        ...
        foreach ($convos as $username) {
            ...
            $style = ($row['opened'] == 'no') ? "background-color: rgba(255, 177, 0, 0.41);" : "";
