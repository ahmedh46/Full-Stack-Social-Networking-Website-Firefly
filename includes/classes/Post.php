...
class Post {
    ...
    public function submitPost($body, $user_to, $imageName) {
        ...
        if ($check_empty != "") {
            ...
            $update_query = mysqli_query($this->con, "UPDATE users SET num_posts=num_posts + 1 WHERE user_name='$added_by'");
            ...
            $stopWords = "a about above across after again against all almost alone along already
			 ...
			 grouping groups h had has have having hi hello he her here herself high high high higher
    ...
    public function loadPostsFriends($data, $limit) {
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            $num_iterations = 0; //Number of results checked (not neccesarily posted)
            $count = 1;
            ...
            while ($row = mysqli_fetch_array($data_query)) {
                ...
                if ($user_logged_obj->isFriend($added_by)) {
                    ...
                    //Once 10 posts have been laoded, break
                    if ($count > $limit) {
                        break;
                    } else {
                        $count++;
                    ...
                    if ($userLoggedIn == $added_by) {
                        $delete_button = "<span> <button class='delete_button btn-danger' id='post$id'>X</button> </span>";
                    } else {
                        $delete_button = "";
                    }
                    ...
                    <script>
                        function toggle<?php echo $id; ?>(event) {
                            ...
                            var comment_click = document.getElementsByClassName("newsfeedPostOptions");
                            var target = $(event.target);
                            if (target.is(comment_click)) {
                    ...
                    $str .= "<div class='status_post' onClick='javascript:toggle$id(event)' style='width: 95%; font-size: 15px; padding: 0px 5px; min-height: 75px;' >
                            <div style='float: left; margin-right: 10px;' class='post_profile_pic'>
                                <img style='border-radius: 30px;' src='$profile_pic' width='50'>
                        </div>
                        
                        <div class='posted_by' style='color:#000; '>
                            <a href='$added_by'> $first_name $last_name </a> $user_to &nbsp; | &nbsp $time_message </span>
                              $delete_button 
                        </div>
                        <div id='post_body'>
                            $body
                            ...
                            <iframe scrolling='no' src='like.php?post_id=$id'> </iframe>    
                            </div>
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            while ($row = mysqli_fetch_array($data_query)) {
                ...
                $str .= "<div class='status_post' onClick='javascript:toggle$id(event)' style='width: 95%; font-size: 15px; padding: 0px 5px; min-height: 75px;' >
                        ...
                        <div class='posted_by' style='color:#000; '>
                            <a href='$added_by'> $first_name $last_name </a>  &nbsp; | &nbsp; $time_message </span>
                             $delete_button
                        </div>
                        <div id='post_body'>
                            $body
    ...
    public function getSinglePost($post_id) {
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            echo "<p>No post found. It may have been deleted by the user or may be broken.</p>";
            return;
        }
